...
function createData(timbrefiscale, tva) {
  return { timbrefiscale, tva };
}
  ...
  return (
    <TableContainer style={{width:200, marginLeft:100, marginBottom:20}} component={Paper}>
      <Table sx={{ maxWidth:500, minWidth:100 }} aria-label="customized table">
        <TableHead>
          <TableRow>
            <StyledTableCell style={{alignContent:"center"}} >Timbre fiscale</StyledTableCell>
            <StyledTableCell style={{alignContent:"center"}}>TVA</StyledTableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row) => (
            <StyledTableRow key={row.id}>
              <StyledTableCell style={{alignContent:"center"}}>{row.timbrefiscale}</StyledTableCell>
              <StyledTableCell style={{alignContent:"center"}}>{row.tva}</StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
